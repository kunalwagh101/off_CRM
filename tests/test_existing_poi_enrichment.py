...
def test_positional_null_match_does_not_shift_email_to_wrong_person(tmp_path: Path):
    ...
    rejection_ledger = tmp_path / "out" / "offsetx_apollo_rejection_ledger.csv"
    assert rejection_ledger.exists()
    rejection_rows = pd.read_csv(rejection_ledger, dtype=str, keep_default_na=False)
    no_match = rejection_rows[rejection_rows["Reason"] == "enriched_no_match"].iloc[0]
    assert no_match["Full Name"] == "First Missing"
    assert no_match["Permanent Exclusion"] == "false"
