...
=== STEP 5 — CLOSE ===
  ...
  5. Commit and push to the owner's authorised working branch. The current
     consolidation uses integration/main-consolidation; promotion to main
     follows review of the combined test evidence.
...
The owner requested one integration branch before promotion to main. Work on
`integration/main-consolidation`, starting from main `37cf9b1`, and combine the
pending recovery, production-contract, PLAN.md and page-cache work there.
Preserve main's authentication, page-reuse and failed-action recovery changes.
Use `BOARD.md` for story status and record combined verification separately
from the historical evidence carried by each input branch.

The merged WP1 configuration requires a mounted persistent data root and passing
checks before deployment. Applying that configuration to a customer service is
a separate operation. A passing integration branch is not a deployment claim.
